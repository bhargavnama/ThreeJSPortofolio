...
const Developer = ({ animationName = "idle", ...props }) => {
  ...
  useEffect(() => {
    if (!actions[animationName]) {
      console.warn(`Animation "${animationName}" not found in actions. Falling back to "idle".`);
    }
    const action = actions[animationName] || actions["idle"];
    if (action) {
      action.reset().fadeIn(0.5).play();
    } else {
      console.warn("Fallback animation \"idle\" not found in actions.");
    }

    return () => {
      const fadeOutAction = actions[animationName] || actions["idle"];
      if (fadeOutAction) {
        fadeOutAction.fadeOut(0.5);
      } else {
        console.warn("FadeOut animation not found for cleanup.");
      }
    };
  }, [animationName]);
  return (
